...
PARQUET_PATH = 'D:/fintech/AML/data/processed/aml_ultra_2m.parquet'
MODEL_DIR = 'D:/fintech/AML/models/saved'

def extract_and_train_2m():
    print("[*] Extracting 2,000,000+ sample with 100% of all laundering schemes across 9.5M rows...")
    laundering_chunks = []
    normal_chunks = []
    total_scanned = 0
    normal_target = 1800000
    normal_collected = 0
    ...
    for chunk in pd.read_csv(CSV_PATH, chunksize=500000):
        ...
        if normal_collected < normal_target:
            sample_size = min(len(chunk), 180000)
            normal = chunk[chunk['Is_laundering'] == 0].sample(n=sample_size, random_state=42)
            normal_chunks.append(normal)
            normal_collected += len(normal)
            
        print(f"   -> Progress: {total_scanned:,} rows scanned (Collected {normal_collected:,} normal, {sum(len(c) for c in laundering_chunks):,} laundering)...")
    ...
    print(f"[+] Total training records assembled: {len(df_full):,}")
    ...
    df_full['z_score_amount'] = np.clip((df_full['Amount'] - df_full['sender_avg_vol']) / (df_full['sender_vol_std'] + 1e-5), -5.0, 15.0)
    ...
    pay_cols = [c for c in df_full.columns if c.startswith('pay_type_')]
    feature_cols.extend(pay_cols)
    
    X = df_full[feature_cols].fillna(0.0)
    y = df_full['Is_laundering'].astype(int)
    ...
    print(f"[*] Training Deep 500-Tree XGBoost Model (scale_pos_weight={scale_pos_weight:.1f}, depth=9)...")
    ...
    metrics = {
        'model_name': 'Deep 500-Tree High-Capacity XGBoost Classifier',
    ...
    print(f"Test Set Evaluated     : {len(X_test):,} transactions")
    ...
    print("[+] Model artifacts updated successfully!")
    return model, metrics

if __name__ == '__main__':
    extract_and_train_2m()
